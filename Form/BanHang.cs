...
namespace WinFormsApp1
{
    ...
    public partial class BanHang : Form
    {
        ...
        private void LoadSanPham()
        {
            ...
            foreach (var sp in danhSach)
            {
                ...
                PictureBox pic = new PictureBox
                {
                    ...
                    Image = Image.FromFile(sp.HinhAnh), // Đảm bảo đường dẫn đúng
                };
        ...
        void ChonPanel(Panel panel, Label lblTen, cVatLieu spchon)
        {
            if (panelDangChon != null)
                panelDangChon.BackColor = System.Drawing.Color.White; // Trả màu cũ

            panel.BackColor = System.Drawing.Color.LightGray; // Màu khi được chọn
            panelDangChon = panel; // Lưu lại panel được chọn\\
            tensp = lblTen.Text;
            masp = spchon.id;
            int maSanPham = spchon.id;
        ...
        {
            cboLoc.ValueMember = "id";
            cboLoc.DisplayMember = "TenSanPham";
            cboLoc.DataSource = context.VatLieu.ToList();
        }
        private void laydanhmuc()
        {
            cboLoc.ValueMember = "id";
            cboLoc.DisplayMember = "TenDanhMuc";
            cboLoc.DataSource = context.DanhMuc.ToList();
        }
        private void layhang()
        {
            cboLoc.ValueMember = "ID";
            cboLoc.DisplayMember = "TenHangSanXuat";
            cboLoc.DataSource = context.HangSanXuat.ToList();
        }
        ...
        private void radhang_CheckedChanged(object sender, EventArgs e)
        {

            BanHang_Load(sender, e);
        }

        private void raddm_CheckedChanged(object sender, EventArgs e)
        {
        ...
        {
            flowLayoutPanel1.Controls.Clear();
            string keyword = cboLoc.Text.Trim();

            List<cVatLieu> danhSach = new List<cVatLieu>();
            if (raddm.Checked)
            {
                danhSach = context.VatLieu.Where(d => d.DanhMuc.TenDanhMuc.Contains(keyword)).ToList();
            }
            else if (radsp.Checked)
            {
                danhSach = context.VatLieu.Where(d => d.TenSanPham.Contains(keyword)).ToList();
            }
            else if (radhang.Checked)
            {
                danhSach = context.VatLieu.Where(d => d.HangSanXuat.TenHangSanXuat.Contains(keyword)).ToList();
            }
            foreach (var sp in danhSach)
            {
                var panel = new Panel
        ...
        private void btnThoat_Click(object sender, EventArgs e)
        {
            ...
            if (kq == DialogResult.Yes)
            {
                ...
                if (daluu == false)
                {
                    if (id != 0)
                    {
                        for (int index = 0; index < soluongbandau.Length; index++)
                        {
                            var bandau = soluongbandau[index];
                            if (index >= dataGridView.Rows.Count) break;
                            var row = dataGridView.Rows[index];

                            if (row.IsNewRow) continue;

                            var sanPhamId = row.Cells["SanPhamID"].Value;
                            var soluongBan = row.Cells["SoLuongBan"].Value;

                            if (sanPhamId != null && soluongBan != null)
                            {
                                int id = Convert.ToInt32(sanPhamId);
                                int soLuongHienTai = Convert.ToInt32(soluongBan);

                                var sp = danhsach.FirstOrDefault(x => x.id == id);
                                if (sp != null)
                                {
                                    sp.SoLuong += (soLuongHienTai - bandau);
                                }
                            }
                        }
                        context.SaveChanges();
        ...
        {
            this.Hide();
            frmHoaDon_ChiTiet ct = new frmHoaDon_ChiTiet(id);
            ct.ShowDialog();
            this.Close();
        ...
        private void numTonKho_ValueChanged(object sender, EventArgs e)
        {
